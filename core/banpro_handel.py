...
class BanproHandle:
    ...
    async def handle_link_recall_warn(
        ...
        if isinstance(mode, bool):
            ...
            await event.send(event.plain_result(f"本群链接撤回禁言后提醒：{mode}"))
        else:
            status = await self.db.get(gid, "link_recall_warn", True)
            await event.send(event.plain_result(f"本群链接撤回禁言后提醒：{status}"))
    ...
    async def _send_link_recall_warning(
        self, event: AiocqhttpMessageEvent, gid: str | int, kicked: bool
    ) -> None:
        """链接被撤回后发送警告语；踢出用户后不再额外提醒。"""
        if kicked or not await self.db.get(gid, "link_recall_warn", True):
            return
